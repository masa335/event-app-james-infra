resource "aws_db_subnet_group" "james_db_subnet_group" {
...
resource "aws_db_instance" "james_db" {
  ...
  vpc_security_group_ids = [aws_security_group.james_sg_db.id]
  db_subnet_group_name   = "${aws_db_subnet_group.james_db_subnet_group.name}"
}